...
impl TradeDirection {
    ...
    pub fn compare_w_u8(&self, other: u8) -> bool {
        match self {
            TradeDirection::ZeroForOne => other == 0,
            TradeDirection::OneForZero => other == 1,
        }
    }

    pub fn to_enum(other: u8) -> TradeDirection {
        match other {
            0 => TradeDirection::ZeroForOne,
            1 => TradeDirection::OneForZero,
...
impl CurveCalculator {
    pub fn validate_supply(token_amount: u64) -> Result<()> {
        if token_amount == 0 {
